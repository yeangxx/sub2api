...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	routingPolicyRepository := repository.NewRoutingPolicyRepository(db)
	upstreamPriceBookRepository := repository.NewUpstreamPriceBookRepository(db)
	routingPolicyControlService := service.NewRoutingPolicyControlService(routingPolicyRepository, upstreamPriceBookRepository)
	repositoryRoutingPriceResolver := service.NewRepositoryRoutingPriceResolver(upstreamPriceBookRepository)
	memoryRoutingHealthStore := service.NewMemoryRoutingHealthStore()
	routingPolicyRuntime := service.NewRoutingPolicyRuntime(routingPolicyControlService, repositoryRoutingPriceResolver, memoryRoutingHealthStore)
	gatewayService := service.ProvideGatewayService(accountRepository, groupRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, identityService, httpUpstream, deferredService, claudeTokenProvider, sessionLimitCache, rpmCache, digestSessionStore, settingService, tlsFingerprintProfileService, channelService, modelPricingResolver, balanceNotifyService, serviceUserPlatformQuotaRepository, routingPolicyRuntime)
	...
	channelMonitorService := service.ProvideChannelMonitorService(channelMonitorRepository, secretEncryptor, routingPolicyRuntime)
	...
	routingPolicyHandler := handler.ProvideRoutingPolicyHandler(routingPolicyControlService, accountRepository, routingPolicyRuntime, secretEncryptor, configConfig)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, complianceHandler, routingPolicyHandler)
